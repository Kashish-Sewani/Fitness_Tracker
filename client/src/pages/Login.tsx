import { AtSignIcon, EyeOffIcon, LockIcon, MailIcon, EyeIcon } from "lucide-react"
    ...
    <>
      ...
      <main className="login-page-container">
        <form onSubmit={handleSubmit} className="login-form">
          ...
          <p className='mt-2 text-sm text-gray-500/90 dark:text-gray-400'>{state === 'login' ? 'Please enter email and password to sign in.' :
          ...
          {state !== 'login' && (
            <div className='mt-4'>
              ...
              dark:text-gray-300'>Username</label>
              <div className='relative mt-2'>
                <AtSignIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 size-4.5" />
                <input onChange={(e) => setUsername(e.target.value)} value={username}
                  type="text" placeholder="Enter username" className="login-input" required />
              </div>
            </div>
          )}
          ...
          {state === 'login' ?
            (
              <p className='text-center py-6 text-sm text-gray-500 dark:text-gray-400'>
                Don't have an account? <button type="button" onClick={() => setState('signup')}
            ...
            (
              <p className='text-center py-6 text-sm text-gray-500 dark:text-gray-400'>
                Already have an account? <button type="button" onClick={() => setState('login')}
                className='ml-1 cursor-pointer text-green-600 
