...
const Dashboard = () => {
  ...
  // Compute todayFood and todayActivities directly from context data
  const { todayFood, todayActivities } = useMemo(() => {
    const today = new Date().toISOString().split('T')[0]
    
    const foodData = allFoodLogs.filter(
      (f: FoodEntry) => f.date === today || f.createdAt?.split('T')[0] === today
    )
    
    const activityData = allActivityLogs.filter(
      (a: ActivityEntry) => a.date === today || a.createdAt?.split('T')[0] === today
    )
    
    return { todayFood: foodData, todayActivities: activityData }
  ...
  return (
    <div className='page-container'>
      ...
      <div className='dashboard-grid'>
        ...
        <Card className='shadow-lg col-span-2'>
          ...
          <div className='mt-4 flex justify-between items-center'>
            ...
            <span className='text-sm text-slate-400'>
              {Math.round(
                (totalCalories / DAILY_CALORIE_LIMIT) * 100
              )}
              %
            </span>
          </div>
          ...
          <Card className='bg-linear-to-r from-slate-800 to-slate-700'>
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <p className='text-whitefont-semibold capitalize'>
        ...
        {/* Quick Summary */}
        <Card>
          <h3 className='font-semibold text-slate-300 dark:text-white mb-4'>Today's Summary</h3>
